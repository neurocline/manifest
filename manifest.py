...
def walk(base_path, path_hashes, manifest):
    ...
    for root, _, files in os.walk(base_path):
        for f in files:
            ...
            progress(num_files=len(manifest), hashed_files=hashed_files, sized_files=sized_files, path=item_path)
...
def find_dups(manifest_path):
    ...
    print(f"{num_dups} duplicated hashes found, {dup_files} duplicated files found", file=report_out)
